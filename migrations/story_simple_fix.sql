-- =====================================================
-- STORY APP - SIMPLE FIX FOR EXISTING DATABASE
-- Run this to make the new code work with your current database
-- =====================================================
...
-- =====================================================
-- STEP 2: Update story_login_logs
-- =====================================================
DO $$
DECLARE
  pk_column TEXT;
BEGIN
  -- Find the primary key column name in story_users
  SELECT column_name INTO pk_column
  FROM information_schema.columns
  WHERE table_name = 'story_users'
  AND column_name IN ('id', 'user_id', 'username')
  LIMIT 1;

  IF pk_column IS NULL THEN
    RAISE NOTICE 'Could not find primary key column in story_users';
    RETURN;
  END IF;
...
-- =====================================================
-- STEP 4: Update story_message_history
-- =====================================================
DO $$
BEGIN
