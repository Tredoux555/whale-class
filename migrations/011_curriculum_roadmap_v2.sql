...
-- This migration updates the curriculum system to support 268 works with full metadata
...
BEGIN
  ...
  -- Rename sequence_order to sequence if needed
  IF EXISTS (SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'curriculum_roadmap' AND column_name = 'sequence_order') 
...
-- ============================================
-- Note: This view will work after the seed script populates curriculum_roadmap
-- We'll create it conditionally to avoid errors if columns don't exist yet
DO $$
BEGIN
  -- Only create view if curriculum_areas table exists
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'curriculum_areas') THEN
    EXECUTE '
    CREATE OR REPLACE VIEW child_curriculum_progress AS
    SELECT 
      c.id AS child_id,
    ...
    LEFT JOIN child_work_completion cwc ON (
      cwc.work_id = cr.id::TEXT OR 
      (cwc.work_id IS NULL AND EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = ''child_work_completion'' 
        AND column_name = ''curriculum_work_id''
      ) AND cwc.curriculum_work_id = cr.id)
    ) AND cwc.child_id = c.id
