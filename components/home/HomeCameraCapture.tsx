// components/home/HomeCameraCapture.tsx
// Camera and video recording for parents to document activities
// Photo capture with canvas, video recording (max 30s)
// Front/back camera switch, preview before save, tag with child/area/work
...
interface HomeCameraCaptureProps {
...
export default function HomeCameraCapture({
  onCapture,
  onCancel,
  childName,
  childId,
  ...
  return (
    <div className="fixed inset-0 bg-black z-50 flex flex-col">
      ...
      {/* Info overlay when captured */}
      {isCaptured && (
        <div className="absolute inset-x-0 bottom-24 flex justify-center">
          <div className="bg-black/70 rounded-lg px-4 py-2 text-white text-sm text-center max-w-xs">
            {childName && <p className="font-semibold">{childName}</p>}
            {workName && <p className="text-xs opacity-80">{workName}</p>}
          </div>
        </div>
      )}
        ...
        {isCaptured ? (
          <div className="flex items-center justify-between px-6 py-4">
            ...
            <button
              ...
              className="flex items-center gap-2 px-8 py-3 bg-emerald-500 text-white rounded-xl font-bold hover:bg-emerald-600 transition-colors"
            >
              <span className="text-xl">✓</span>
              <span>Save {captureMode === 'photo' ? 'Photo' : 'Video'}</span>
            </button>
          </div>
        ) : (
          <div className="flex items-center justify-between px-6 py-4">
