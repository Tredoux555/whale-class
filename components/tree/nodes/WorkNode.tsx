...
function WorkNode({ data, selected }: NodeProps<WorkNodeData>) {
  const { work, areaColor, isSelected, onClick, progressStatus } = data;
  ...
  return (
    <div
      className={`work-node ${isSelected ? 'selected' : ''} ${hasPrerequisites ? 'has-prereqs' : ''} ${progressStatus ? `progress-${progressStatus}` : ''}`}
      style={{
        borderColor,
        backgroundColor,
      ...
      <div className="work-node-content">
        ...
        <div className="work-meta">
          ...
          {progressStatus === 'completed' && (
            <span className="work-progress" title="Completed">
              ✓
            </span>
          )}
          {progressStatus === 'in_progress' && (
            <span className="work-progress" title="In Progress">
              ⏳
            </span>
          )}
        </div>
