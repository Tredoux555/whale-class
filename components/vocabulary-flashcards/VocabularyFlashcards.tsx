// @ts-nocheck
// components/vocabulary-flashcards/VocabularyFlashcards.tsx
// Vocabulary Flashcard Maker. Lifted verbatim out of
// /montree/library/tools/vocabulary-flashcards so the Picture Library hub can
// embed it as a tab; that route is now a thin wrapper around this component.
"use client";
...
/** Same shape the Picture Bank puts on the sessionStorage export pipe. */
...
interface VocabularyFlashcardsProps {
   ...
   * Library hub). Only ids not already imported are added, so tab flips and
   * re-renders never duplicate a flashcard.
   */
  importPhotos?: ImportPhoto[];
  /**
   * Render for embedding: drops the sticky page header (back button + title)
   * and the full-page gradient background. The standalone route leaves this
   * off and gets the page exactly as before.
   */
  embedded?: boolean;
}

const VocabularyFlashcardGenerator = ({ importPhotos, embedded = false }: VocabularyFlashcardsProps = {}) => {
  ...
  useEffect(() => {
    try {
      ...
      photos.forEach(p => { if (p.id) importedPhotoIdsRef.current.add(p.id); });
      loadPhotos();
    } catch (err) {
  ...
  // Import photos pushed in by the Picture Library hub. Same conversion as the
  // sessionStorage pipe above, but deduped by photo id so flipping away and
  // back never re-adds a card the user already removed.
  useEffect(() => {
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  return (
    <div className={embedded ? '' : 'min-h-screen bg-gradient-to-b from-cyan-50 to-blue-50'}>
      {/* Sticky header — the host page owns the chrome when embedded */}
      {!embedded && (
