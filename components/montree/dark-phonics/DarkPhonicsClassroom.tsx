// components/montree/dark-phonics/DarkPhonicsClassroom.tsx
// Dark Phonics — the whole programme on one page ("classroom media").
//
// 🚨 THIS FILE WAS app/montree/library/dark-phonics/page.tsx AND STILL IS ITS
// WHOLE BODY. It moved on 2026-09-17 so that TWO surfaces can render it:
//   - /montree/library/dark-phonics — the library page, now a four-line mount
//     of this component, rendering byte-identically to before;
//   - /dark-phonics?tab=classroom — the public hub's Classroom tab, which
//     passes `embedded` to drop the library chrome it already provides.
// Nothing else changed in the move: the data, the tabs, the pills, the two
// tracks and the A5 rules are exactly as they were.
//
...
export interface DarkPhonicsClassroomProps {
  /**
   * Rendered inside another page's chrome (the /dark-phonics hub's Classroom
   * tab). Drops the "← Library" link and the language toggle — the host header
   * already carries both — and lets the host own the page background and the
   * viewport height. EVERY piece of curriculum content is unchanged: same
   * cards, same tabs, same pills, same order.
   */
  embedded?: boolean;
}

export default function DarkPhonicsClassroom({ embedded = false }: DarkPhonicsClassroomProps) {
  ...
  return (
    <div
      className={`relative overflow-hidden flex flex-col ${embedded ? '' : 'min-h-screen'}`}
      style={{ background: '#06140e' }}
    >

      <div aria-hidden="true" style={{
        position: embedded ? 'absolute' : 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none',
        background: `
      ...
      >
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 sm:gap-3 w-full sm:w-auto">
          ...
          {/* The pill is ~26px tall; the wrapper carries the 44px tap floor.
              Hidden when embedded: the hub header has its own EN/中 toggle and
              two language switches on one screen is a bug, not a feature. */}
          {embedded ? null : (
      ...
      <div
        ...
        style={{
          // Embedded, the hub's own sticky header owns the top of the viewport,
          // so the jump box starts below it rather than under it.
          top:
            navHeight !== null
              ? `${navHeight + (embedded ? 68 : 8)}px`
              : `calc(env(safe-area-inset-top) + ${embedded ? '9rem' : '4.5rem'})`,
        }}
      >
