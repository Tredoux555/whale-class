...
// Number tracing game for 0-9 - based on LetterTraceGame

'use client';

import React, { useState, useRef, useEffect, useCallback } from 'react';
import { useRouter } from 'next/navigation';
import { NUMBER_DATA, NUMBER_STROKES } from '@/lib/games/number-strokes';
import { StrokePoint, STROKE_TOLERANCE, validateStroke } from '@/lib/games/letter-strokes';
import { GameAudio } from '@/lib/games/audio-paths';
import Confetti from './Confetti';

const PROGRESS_KEY = 'number-trace-progress';
...
export default function NumberTraceGame() {
  const router = useRouter();
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const animationRef = useRef<number | null>(null);
  const isAnimatingRef = useRef(false);

  const [progress, setProgress] = useState<TraceProgress>({ completedNumbers: [] });
  ...
  const currentNumber = NUMBER_DATA[numberIndex];
  const strokeData = NUMBER_STROKES[currentNumber.number];

  // Load progress and check for student session
  useEffect(() => {
  ...
  const startGameSession = async (cId: string) => {
    try {
      const res = await fetch('/api/games/progress', {
        ...
        body: JSON.stringify({ action: 'start', childId: cId, gameId: 'number-tracer', gameName: 'Number Tracer' })
      });
  ...
  const endGameSession = useCallback(async () => {
    ...
    try {
      await fetch('/api/games/progress', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'end', sessionId, itemsCompleted: progress.completedNumbers.length, itemsTotal: 10, itemsMastered: progress.completedNumbers.map(String) })
      });
    } catch (e) { console.error(e); }
  }, [sessionId, progress.completedNumbers]);
  ...
  const handlePointerUp = () => {
    ...
    if (isValid) {
      ...
      if (currentStrokeIndex >= strokeData.strokes.length - 1) {
        ...
        const newCompleted = [...new Set([...progress.completedNumbers, currentNumber.number])];
        saveProgress({ completedNumbers: newCompleted });
        setTimeout(() => {
          setShowConfetti(false);
          if (numberIndex < NUMBER_DATA.length - 1) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl p-8 text-center max-w-md shadow-2xl">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Amazing!</h1>
          <p className="text-gray-600 mb-6">You traced all the numbers!</p>
          <div className="flex gap-4 justify-center">
            <button onClick={() => { setNumberIndex(0); resetNumber(); }} className="px-6 py-3 bg-purple-500 text-white rounded-xl font-bold">Play Again</button>
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-white text-center">
            <div className="text-2xl font-bold">Number {currentNumber.number}</div>
            <div className="text-white/80 text-sm">{numberIndex + 1} / 10</div>
          </div>
          <div className="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center text-white font-bold">
            {progress.completedNumbers.length}
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-6 shadow-xl mb-4">
          <div className="text-center mb-4">
            <span className="text-6xl">{currentNumber.image}</span>
            <div className="text-gray-600 mt-2 capitalize">{currentNumber.word}</div>
          </div>
        ...
        <div className="bg-white/20 rounded-full h-3 overflow-hidden">
          <div className="bg-white h-full transition-all" style={{ width: `${(progress.completedNumbers.length / 10) * 100}%` }} />
        </div>
      </div>
    </div>
  );
