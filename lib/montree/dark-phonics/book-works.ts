 ...
 * own curriculum data and live in ./book-works-lessons.ts (first language) and
 * ./book-works-lessons.second-language.ts (the simple wording the shelf now
 * ships — see BOOK_TRACK below) — see either file's
...
/**
 * 🚨 WHICH WORDING THE DIGITAL SHELF READS (2026-09-15, per Tredoux).
 *
 * The same standing rule that put `PRINTABLE_TRACK = 'second'` on the library
 * page (app/montree/library/dark-phonics/page.tsx) governs here: the book the
 * child is shown is the SIMPLE one — the version the teacher actually reads in
 * class, every sentence four words or fewer. Print and the shelf must not
 * disagree about what a book says, so the shelf reads the SECOND-LANGUAGE pack.
 *
 * For lesson 5 (the-pit) that is the whole point of the change: the shelf used
 * to serve "The ant sat in the… pit!", a wording the printed second-language
 * book retired, and now serves "Ant in the… pit!" — the line on the page the
 * teacher holds.
 *
 * NOTHING WAS DELETED. `book-works-lessons.ts` is the first-language pack, is
 * still complete, still generated by the same pipeline, and is still imported
 * here — flip BOOK_TRACK back to 'first' and the shelf is exactly what it was.
 * The two packs are structurally identical (same lessons, same cast, same art,
 * same rounds, same questions, same script); only the sentences differ, and on
 * six books — lessons 3, 4, 6, 9, 17 and 21 — not even those, because the
 * second-language transform is a no-op on them (their printed A5 readers are
 * byte-identical across the two tracks).
 *
 * The conformance test follows this constant: side-cars are committed per
 * track under scripts/curriculum/book-works/sidecars[/second-language]/, and
 * tests/dark-phonics/book-works-sidecar-conformance.test.ts reads the tree
 * that matches whatever the shelf ships.
 */
export type BookTrack = 'first' | 'second';
export const BOOK_TRACK: BookTrack = 'second';
...
const LESSON_1: BookWorksLesson = {
  ...
  pages: [
    ...
    // Second language, matching the printed 2L reader: the roll-call recap is a
    // three-times chant (see BOOK_TRACK above).
    { art: `${SOCK}/p6-recap.png`, sentence: 'In my sock?! In my sock?! In my sock?!', chant: true },
    { art: `${SOCK}/p7-potato-twist.png`, sentence: 'Potato in my sock?' },
  ],
