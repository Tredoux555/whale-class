...
export const CULTURAL_SIGNATURES: WorkSignature[] = [
...

  {
    ...
    category: 'Physical Science',
    visual_description:
      'HANDS-ON COLOR THEORY exploration with PRIMARY COLORS and COLOR MIXING discovery. Setup includes: THREE PRIMARY COLORS in distinct forms - RED (vibrant red), YELLOW (bright yellow), BLUE (pure blue) available as LIQUID PAINT or WATER-BASED DYES in clear containers or as thick WATERCOLOR paints in cake form. Child COMBINING primary colors in shallow trays, cups, or on mixing palettes to CREATE SECONDARY COLORS: RED + YELLOW = ORANGE (bright orange, distinct from either parent color), YELLOW + BLUE = GREEN (lush green, clearly different from parent colors), RED + BLUE = PURPLE/VIOLET (distinct purple hue). Activities include: MIXING PAINTS in shallow trays or ceramic palettes - child uses paintbrush or spoon to combine colors, STIRRING to create complete mixture, OBSERVING the color CHANGE in real-time (immediate gratification of color transformation), RECORDING results on a COLOR MIXING CHART with color circles showing before and after, optional TISSUE PAPER or CELLOPHANE OVERLAYS showing color mixing by light transmission (when transparent colored sheets overlap, colors appear to mix). Child CREATING a COLOR WHEEL by PAINTING or PLACING color circles (reds at top, yellows to right, blues to left, secondary colors between). Optional WHITE and BLACK for tints (lightening with white) and shades (darkening with black). From 1-2 meters: shallow containers of colored liquids, mixed results showing new colors, color wheel or chart visible. NOT complex color theory (stays with PRIMARY and SECONDARY colors only, no tertiary or complementary color complexity). NOT painting as art (focuses on SCIENTIFIC color observation and mixing, not artistic expression - though child may enjoy the beauty).',
    key_materials: ['Primary color paints or dyes (red, yellow, blue)', 'Watercolor cakes (alternative)', 'Mixing trays or ceramic palettes', 'Paintbrushes or stirring spoons', 'White paper or color mixing chart', 'Color mixing wheel template (optional)', 'Optional: tissue paper overlays'],
    confusion_pairs: [],
    difficulty: 'easy'
  },
