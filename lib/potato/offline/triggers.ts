// lib/potato/offline/triggers.ts
// When the queue wakes up.
//
// Ported from lib/montree/offline/sync-triggers.ts, minus the Capacitor
// branches — Potato Snaps is a web/PWA product and may not import
// lib/montree/platform. The web events cover both the browser and the
// installed PWA.
//
// Four triggers:
...
//   4. one pass shortly after registration, for photos left over from last time
...
export function registerPotatoSyncTriggers(classId: string): () => void {
  ...
  const run = (why: string) => {
    syncQueue(classId)
      .catch((e) => console.error(`[potato/queue] sync (${why}) failed:`, e))
   ...
   * it a failed photo would sit until the teacher happened to switch tabs.
   */
  function scheduleNextDue() {
    ...
    nextDueAt(classId)
      ...
      .catch((e) => console.error('[potato/queue] scheduling failed:', e));
  }
  ...
  const sweep = setInterval(() => {
    getQueueSize()
      ...
      .catch((e) => console.error('[potato/queue] sweep failed:', e));
  }, 30 * 60 * 1000);
