// lib/potato/offline/queue-store.ts
// IndexedDB persistence for the capture queue.
//
// Ported from lib/montree/offline/queue-store.ts. IndexedDB rather than SQLite
// for the same reasons it was chosen there: it exists in every browser and in
// the installed PWA, it stores Blobs without base64, it needs no native plugin,
// and it survives an app restart. For a queue of a few hundred items it is
// plenty.
//
...
const DB_NAME = 'potato-snaps-queue';
const DB_VERSION = 1;
...
function normalizeIDBError(err: unknown, context: string): Error {
  ...
  return new Error(
    `Photo storage failed (${context}) — the browser gave no detail ` +
...
function openDB(): Promise<IDBDatabase> {
  ...
  dbPromise = new Promise((resolve, reject) => {
    if (typeof indexedDB === 'undefined') {
      dbPromise = null;
      reject(new Error('This browser can’t save photos on the device.'));
      return;
    }
    ...
    try {
      ...
      console.error('[potato/queue] indexedDB.open threw:', err);
      dbPromise = null;
      reject(new Error('Photo storage unavailable — are you in private browsing?'));
      return;
    }

    request.onerror = () => {
      console.error('[potato/queue] open failed:', request.error);
      dbPromise = null;
 ...
 */
export async function saveEntryAndBlob(entry: QueueEntry, blob: Blob): Promise<void> {
...
/**
 * Entries that are due for an attempt right now, oldest capture first.
 * `nextAttemptAt` is what implements the exponential backoff: a failed entry
 * simply is not due yet.
 */
export async function getDueEntries(classId: string, now = Date.now()): Promise<QueueEntry[]> {
...
export async function nextDueAt(classId: string): Promise<number | null> {
  const all = await getAllEntries();
  const times = all
    .filter((e) => (e.status === 'pending' || e.status === 'failed') && e.classId === classId)
    .map((e) => new Date(e.nextAttemptAt).getTime())
    .filter((t) => Number.isFinite(t));
  return times.length > 0 ? Math.min(...times) : null;
}

export async function findByContentHash(contentHash: string, classId: string): Promise<QueueEntry | null> {
  const db = await openDB();
  return new Promise((resolve, reject) => {
    const tx = db.transaction(STORE_ENTRIES, 'readonly');
    ...
    request.onsuccess = () => {
      ...
      resolve(matches.find((e) => e.classId === classId) ?? null);
    };
  });
}

export async function getStats(classId?: string): Promise<QueueStats> {
  const all = await getAllEntries();
  const mine = classId ? all.filter((e) => e.classId === classId) : all;
 ...
 *
 * 🚨 DELIBERATELY WEAKER THAN MONTREE'S. Montree's aggressiveCleanup will, as a
 * last resort, delete pending entries older than 7 days. Potato Snaps must not:
 * the founder's rule is that a photo saved to the device is never silently
 * dropped. So only entries the server already has, or has permanently refused
 * (and which the teacher can see and dismiss), are ever removed here. If that
 * is not enough room, enqueue fails loudly instead.
 */
export async function makeRoom(): Promise<number> {
  ...
  // Rejections older than a week: the teacher has had every chance to see them.
...
/** Drop a rejected entry the teacher has chosen to give up on. */
