# 🌿 Sanctuary — Master Project Doc (START HERE)

**Sanctuary** is a native iOS app: a private, on-device-encrypted **journal +
...
> "Sanctuary"** everywhere the user sees it.
...
| Folder | What |
|---|---|
| `project.yml` | XcodeGen spec (target **Sanctuary**, bundle `xyz.montree.sanctuary`, swift-sodium) |
...
| Run `SanctuaryTests` (crypto parity) → green | 15 min |
...
**Create a Sanctuary account** (one per person; username = the name typed at the
...
3. `brew install xcodegen && cd native/Sanctuary && xcodegen generate && open Sanctuary.xcodeproj`
4. Set Team + signing. Resolve swift-sodium. Fix first-build errors (README_BUILD §6).
5. Run `SanctuaryTests` → all green (crypto parity with the server).
