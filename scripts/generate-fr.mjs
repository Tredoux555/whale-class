#!/usr/bin/env node
// Generates lib/montree/i18n/fr.ts from en.ts using Anthropic API (Haiku)
// Run: node generate-fr.mjs
...
const OUT_PATH = path.join(WHALE_PATH, 'lib/montree/i18n/fr.ts');
...
async function translateBatch(pairs) {
  ...
  const systemPrompt = `You are a professional French translator for a Montessori school management app called Montree.

RULES:
1. Translate each line's VALUE to French. Keep the KEY before ||| unchanged.
2. Formal vous register throughout (vous, votre, vos — NEVER tu/ton/ta)
3. Use "votre enfant" for "your child"
4. AMI Montessori area terms: Vie Pratique, Sensoriel, Mathématiques, Langage, Culture
...
10. Output format: KEY|||FRENCH_TRANSLATION (one per line, no extra text)
11. For app.name or similar brand names, keep them as-is
12. Short UI strings like "Save", "Cancel", "Loading..." translate normally`;

  const userMsg = `Translate these key|||value pairs. Output exactly the same number of lines as input, same format KEY|||FRENCH_VALUE:\n\n${inputLines}`;
