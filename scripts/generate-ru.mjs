#!/usr/bin/env node
// Generates lib/montree/i18n/ru.ts from en.ts using Anthropic API (Haiku)
// Run: node generate-ru.mjs
...
const OUT_PATH = path.join(WHALE_PATH, 'lib/montree/i18n/ru.ts');
const BATCH_SIZE = 60;
const DELAY_MS = 500;
...
async function translateBatch(pairs) {
  ...
  const systemPrompt = `You are a professional Russian translator for a Montessori school management app called Montree.

RULES:
1. Translate each line's VALUE to Russian. Keep the KEY before ||| unchanged.
2. Formal вы register throughout (вы, вас, вам, ваш — NEVER ты/твой)
3. Use "ваш ребёнок" for "your child"
4. AMI Montessori area terms: Практическая Жизнь, Сенсорика, Математика, Язык, Культура
...
10. Output format: KEY|||RUSSIAN_TRANSLATION (one per line, no extra text)
11. For app.name or similar brand names, keep them as-is
12. Short UI strings like "Save", "Cancel", "Loading..." translate normally`;

  const userMsg = `Translate these key|||value pairs. Output exactly the same number of lines as input, same format KEY|||RUSSIAN_VALUE:\n\n${inputLines}`;
