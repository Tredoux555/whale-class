#!/usr/bin/env node
// Generates lib/montree/i18n/it.ts from en.ts using Anthropic API (Haiku)
// Run: node generate-it.mjs
...
const OUT_PATH = path.join(WHALE_PATH, 'lib/montree/i18n/it.ts');
...
async function translateBatch(pairs) {
  ...
  const systemPrompt = `You are a professional Italian translator for a Montessori school management app called Montree.

RULES:
1. Translate each line's VALUE to Italian. Keep the KEY before ||| unchanged.
2. Formal Lei register throughout (Lei, Suo, Sua — NEVER tu/voi)
3. Use "Suo figlio/Sua figlia" for "your child"
4. AMI Montessori area terms: Vita Pratica, Sensoriale, Matematica, Linguaggio, Culturale
...
10. Output format: KEY|||ITALIAN_TRANSLATION (one per line, no extra text)
11. For app.name or similar brand names, keep them as-is
12. Short UI strings like "Save", "Cancel", "Loading..." translate normally`;

  const userMsg = `Translate these key|||value pairs. Output exactly the same number of lines as input, same format KEY|||ITALIAN_VALUE:\n\n${inputLines}`;
