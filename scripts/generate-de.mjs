#!/usr/bin/env node
// Generates lib/montree/i18n/de.ts from en.ts using Anthropic API (Haiku)
// Run: node generate-de.mjs
...
const DE_PATH = path.join(WHALE_PATH, 'lib/montree/i18n/de.ts');
...
// Parse all key-value pairs from en.ts
...
async function translateBatch(pairs) {
  ...
  const systemPrompt = `You are a professional German translator for a Montessori school management app called Montree.

RULES:
1. Translate each line's VALUE to German. Keep the KEY before ||| unchanged.
2. Formal Sie register throughout (Sie, Ihr, Ihnen — NEVER du/dein/dir)
3. Use "Ihr Kind" for "your child" (never "dein Kind")
4. AMI Montessori area terms: Praktisches Leben, Sinnesmaterial, Mathematik, Sprache, Kulturelles
...
10. Output format: KEY|||GERMAN_TRANSLATION (one per line, no extra text)
11. For app.name or similar brand names, keep them as-is
12. Short UI strings like "Save", "Cancel", "Loading..." translate normally`;

  const userMsg = `Translate these key|||value pairs. Output exactly the same number of lines as input, same format KEY|||GERMAN_VALUE:\n\n${inputLines}`;
...
async function main() {
  ...
  // Walk original pairs to preserve order
  ...
  fs.writeFileSync(DE_PATH, lines.join('\n'), 'utf-8');
