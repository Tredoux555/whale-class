// scripts/audit-non-jpeg-photos.mjs
// READ-ONLY audit — does NOT delete anything.
// Identifies montree_media rows that are PHOTOS but not JPEGs.
//
// Heuristic — flag a row as "non-JPEG photo" if ALL of:
//   - media_type IS NULL OR media_type IN ('image', 'photo')   (not explicitly video/audio)
//   - AND any of:
//       - filename does not end with .jpg/.jpeg
//       - storage_path does not end with .jpg/.jpeg
//       - mime_type is set and is not image/jpeg
//
// We treat media_type 'photo' as the canonical photo value (set by /api/montree/media/upload),
// but also include 'image' as a safety net for any older legacy rows.
//
// Run with:
//   node --env-file=.env.local scripts/audit-non-jpeg-photos.mjs

import { createClient } from '@supabase/supabase-js';

const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY;

if (!supabaseUrl || !serviceKey) {
  console.error('Missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY');
  process.exit(1);
}

const sb = createClient(supabaseUrl, serviceKey, {
  auth: { persistSession: false, autoRefreshToken: false },
});

const ALLOWED_EXT = new Set(['jpg', 'jpeg']);

function lastExt(s) {
  if (!s || typeof s !== 'string') return '';
  const i = s.lastIndexOf('.');
  if (i === -1) return '';
  return s.slice(i + 1).toLowerCase().replace(/[?#].*$/, '');
}

function isJpegMime(m) {
  if (!m) return null; // null = unknown, not a fail signal on its own
  const lc = String(m).toLowerCase();
  return lc === 'image/jpeg' || lc === 'image/jpg';
}

async function main() {
  console.log('Connecting to', supabaseUrl);

  // First: discover what columns exist (in case the schema has 'filename' or similar)
  const probe = await sb
    .from('montree_media')
    .select('*')
    .limit(1);

  if (probe.error) {
    console.error('Probe failed:', probe.error.message);
    process.exit(1);
  }

  const sampleRow = probe.data?.[0] || {};
  const cols = Object.keys(sampleRow);
  console.log('\nColumns on montree_media:', cols.join(', '));
  // Real column is file_name (not filename) per the schema. Keep all variants for safety.
  const hasFileName = cols.includes('file_name');
  const hasFilename = cols.includes('filename');
  const hasMimeType = cols.includes('mime_type');
  const hasOriginalFilename = cols.includes('original_filename');

  // Pull all photos in batches.
  // We use cursor-style pagination on captured_at to avoid count-mismatch issues at scale.
  const PAGE = 1000;
  let total = 0;
  let nonJpegRows = [];
  let from = 0;
  while (true) {
    const fields = [
      'id',
      'school_id',
      'classroom_id',
      'child_id',
      'media_type',
      'storage_path',
      'thumbnail_path',
      'captured_at',
      'created_at',
      'caption',
      hasFileName ? 'file_name' : null,
      hasFilename ? 'filename' : null,
      hasOriginalFilename ? 'original_filename' : null,
      hasMimeType ? 'mime_type' : null,
    ]
      .filter(Boolean)
      .join(', ');

    const { data, error } = await sb
      .from('montree_media')
      .select(fields)
      .or('media_type.is.null,media_type.eq.image,media_type.eq.photo')
      .order('captured_at', { ascending: false })
      .range(from, from + PAGE - 1);

    if (error) {
      console.error('Query failed:', error.message);
      process.exit(1);
    }

    if (!data || data.length === 0) break;
    total += data.length;

    for (const row of data) {
      const mime = hasMimeType ? row.mime_type : null;
      const fname = (hasFileName ? row.file_name : null)
        || (hasFilename ? row.filename : null)
        || (hasOriginalFilename ? row.original_filename : null)
        || null;

      const storagePathExt = lastExt(row.storage_path);
      const filenameExt = lastExt(fname);

      const mimeIsJpeg = isJpegMime(mime); // null | true | false
      const storagePathIsJpeg = ALLOWED_EXT.has(storagePathExt);
      const filenameIsJpeg = fname ? ALLOWED_EXT.has(filenameExt) : null; // null = unknown

      // Flag if ANY signal is non-JPEG (and at least one negative signal exists)
      const negative =
        mimeIsJpeg === false ||
        (!storagePathIsJpeg && storagePathExt !== '') ||
        (filenameIsJpeg === false);

      if (negative) {
        nonJpegRows.push({
          id: row.id,
          school_id: row.school_id,
          classroom_id: row.classroom_id,
          child_id: row.child_id,
          media_type: row.media_type,
          storage_path: row.storage_path,
          thumbnail_path: row.thumbnail_path,
          captured_at: row.captured_at,
          created_at: row.created_at,
          mime_type: mime,
          filename: fname,
          storage_path_ext: storagePathExt,
          filename_ext: filenameExt,
        });
      }
    }

    if (data.length < PAGE) break;
    from += PAGE;
  }

  console.log(`\nScanned ${total} candidate rows (media_type IS NULL OR IN ('image','photo'))`);
  console.log(`Non-JPEG photo rows: ${nonJpegRows.length}\n`);

  if (nonJpegRows.length === 0) {
    console.log('Nothing to clean up.');
    return;
  }

  // Breakdown by extension / mime
  const byMime = {};
  const byStoragePathExt = {};
  const byFilenameExt = {};

  for (const r of nonJpegRows) {
    const m = r.mime_type || '(null)';
    byMime[m] = (byMime[m] || 0) + 1;

    const sx = r.storage_path_ext || '(none)';
    byStoragePathExt[sx] = (byStoragePathExt[sx] || 0) + 1;

    const fx = r.filename_ext || '(none)';
    byFilenameExt[fx] = (byFilenameExt[fx] || 0) + 1;
  }

  console.log('Breakdown by mime_type:');
  for (const [k, v] of Object.entries(byMime).sort((a, b) => b[1] - a[1])) {
    console.log(`  ${k}: ${v}`);
  }

  console.log('\nBreakdown by storage_path extension:');
  for (const [k, v] of Object.entries(byStoragePathExt).sort((a, b) => b[1] - a[1])) {
    console.log(`  .${k}: ${v}`);
  }

  console.log('\nBreakdown by filename extension:');
  for (const [k, v] of Object.entries(byFilenameExt).sort((a, b) => b[1] - a[1])) {
    console.log(`  .${k}: ${v}`);
  }

  // Hydrate child names for samples
  const sampleIds = nonJpegRows.slice(0, 12);
  const childIds = [...new Set(sampleIds.map(r => r.child_id).filter(Boolean))];
  const childNameMap = {};
  if (childIds.length > 0) {
    const { data: children } = await sb
      .from('montree_children')
      .select('id, name')
      .in('id', childIds);
    for (const c of children || []) {
      childNameMap[c.id] = c.name;
    }
  }

  console.log('\nSample rows (first 12):');
  for (const r of sampleIds) {
    const name = r.child_id ? (childNameMap[r.child_id] || '(unknown child)') : '(no child)';
    console.log(`  id=${r.id}`);
    console.log(`    captured_at=${r.captured_at || r.created_at}`);
    console.log(`    classroom_id=${r.classroom_id}  child=${name}`);
    console.log(`    storage_path=${r.storage_path}`);
    console.log(`    mime_type=${r.mime_type || '(null)'}  filename=${r.filename || '(null)'}`);
    console.log('');
  }

  // Write all non-JPEG rows to a JSON file for the deletion script
  const fs = await import('node:fs');
  const outPath = '/tmp/non_jpeg_photo_rows.json';
  fs.writeFileSync(outPath, JSON.stringify(nonJpegRows, null, 2));
  console.log(`\nFull row dump written to ${outPath} (${nonJpegRows.length} rows)`);
}

main().catch(err => {
  console.error('Audit failed:', err);
  process.exit(1);
});
