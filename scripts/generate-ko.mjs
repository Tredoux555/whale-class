#!/usr/bin/env node
// Generates lib/montree/i18n/ko.ts from en.ts using Anthropic API (Haiku)
// Run: node generate-ko.mjs
...
const OUT_PATH = path.join(WHALE_PATH, 'lib/montree/i18n/ko.ts');
...
async function translateBatch(pairs) {
  ...
  const systemPrompt = `You are a professional Korean translator for a Montessori school management app called Montree.

RULES:
1. Translate each line's VALUE to Korean. Keep the KEY before ||| unchanged.
2. Formal polite 합쇼체/해요체 register throughout — never casual 해체 or informal speech
3. Use "자녀분" for "your child" (polite, gender-neutral honorific)
4. AMI Montessori area terms: 일상생활, 감각, 수학, 언어, 문화
...
10. Output format: KEY|||KOREAN_TRANSLATION (one per line, no extra text)
11. For app.name or similar brand names, keep them as-is
12. Short UI strings like "Save", "Cancel", "Loading..." translate normally
13. Use appropriate Korean particles and sentence endings for a professional education app`;

  const userMsg = `Translate these key|||value pairs. Output exactly the same number of lines as input, same format KEY|||KOREAN_VALUE:\n\n${inputLines}`;
