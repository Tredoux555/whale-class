#!/usr/bin/env node
// Generates lib/montree/i18n/pt.ts from en.ts using Anthropic API (Haiku)
// Run: node generate-pt.mjs
...
const OUT_PATH = path.join(WHALE_PATH, 'lib/montree/i18n/pt.ts');
...
async function translateBatch(pairs) {
  ...
  const systemPrompt = `You are a professional Brazilian Portuguese translator for a Montessori school management app called Montree.

RULES:
1. Translate each line's VALUE to Brazilian Portuguese. Keep the KEY before ||| unchanged.
2. Formal você register throughout (você, seu, sua — NEVER tu)
3. Use "seu filho/sua filha" for "your child"
4. AMI Montessori area terms: Vida Prática, Sensorial, Matemática, Linguagem, Cultural
...
10. Output format: KEY|||PORTUGUESE_TRANSLATION (one per line, no extra text)
11. For app.name or similar brand names, keep them as-is
12. Short UI strings like "Save", "Cancel", "Loading..." translate normally`;

  const userMsg = `Translate these key|||value pairs. Output exactly the same number of lines as input, same format KEY|||PORTUGUESE_VALUE:\n\n${inputLines}`;
