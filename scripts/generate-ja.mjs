#!/usr/bin/env node
// Generates lib/montree/i18n/ja.ts from en.ts using Anthropic API (Haiku)
// Run: node generate-ja.mjs
...
const OUT_PATH = path.join(WHALE_PATH, 'lib/montree/i18n/ja.ts');
...
async function translateBatch(pairs) {
  ...
  const systemPrompt = `You are a professional Japanese translator for a Montessori school management app called Montree.

RULES:
1. Translate each line's VALUE to Japanese. Keep the KEY before ||| unchanged.
2. Polite desu/masu register throughout — never casual plain form in UI text
3. Use "お子さま" for "your child" (honorific, gender-neutral)
4. AMI Montessori area terms: 日常生活, 感覚, 算数, 言語, 文化
...
10. Output format: KEY|||JAPANESE_TRANSLATION (one per line, no extra text)
11. For app.name or similar brand names, keep them as-is
12. Short UI strings like "Save", "Cancel", "Loading..." translate normally
13. Use natural Japanese word order and particles correctly`;

  const userMsg = `Translate these key|||value pairs. Output exactly the same number of lines as input, same format KEY|||JAPANESE_VALUE:\n\n${inputLines}`;
