// scripts/test-montree-ai.js
// Test script for Montree AI endpoints
// Run: node scripts/test-montree-ai.js
...
async function checkFoundationTables() {
  ...
  if (classrooms?.length) {
    classrooms.forEach(c => console.log(`   - ${c.name} (${c.school?.name || 'no school'})`));
  }
  ...
  if (assignments?.length) {
    ...
    assignments.slice(0, 3).forEach(a => {
      console.log(`     - ${a.child?.name || 'unknown'}: ${a.work?.name || 'unknown'} (${a.status})`);
    });
  }
...
async function testAnalyzeEndpoint(childId) {
  ...
  try {
    ...
    console.log('\nFallback Used:', data._meta?.ai_fallback_used || false);
...
async function testWeeklyReportEndpoint(childId) {
...
async function testSuggestNextEndpoint(childId) {
