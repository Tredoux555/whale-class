...
async function main() {
  ...
  if (OFFLINE) {
    ...
    const esf = model.rows.find(r => (r.email||'').toLowerCase() === 'info@montessori-esf.org');
    console.log('\n-- ESF hub row trace --');
    console.log(JSON.stringify({org_name: esf.org_name, email: esf.email, notes: esf.notes, segment: esf.segment, verification_status: esf.verification_status}, null, 2));
    const famm = model.rows.find(r => (r.email||'').toLowerCase() === 'marisa@fundacionmontessori.org');
    console.log('\n-- FAMM hub row trace --');
    console.log(JSON.stringify({org_name: famm.org_name, email: famm.email, notes: famm.notes, segment: famm.segment, verification_status: famm.verification_status, warm_path: famm.warm_path, priority: famm.priority, website: famm.website, responsiveness_score: famm.responsiveness_score, montessori_verified: famm.montessori_verified}, null, 2));
