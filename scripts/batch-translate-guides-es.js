...
const TRANSLATE_TOOL = {
  name: "save_spanish_guide",
  description: "Save the Argentine Spanish translation of a Montessori Quick Guide",
  input_schema: {
    type: "object",
    properties: {
      quick_guide: { type: "string", description: "Argentine Spanish translation of the quick guide summary" },
...
async function translateWork(work) {
  ...
  return retry(async () => {
    const resp = await anthropic.messages.create({
      ...
      tools: [TRANSLATE_TOOL], tool_choice: { type: "tool", name: "save_spanish_guide" },
      messages: [{ role: "user", content: `Translate this Montessori work guide from English to natural Argentine Spanish / Latin American Spanish. Use warm, accessible language appropriate for parents and teachers in a Montessori school. Keep standard Montessori terms (like "trabajo", "ambiente preparado") where they are commonly used in Spanish-speaking Montessori communities.\n\n${JSON.stringify(fields)}` }],
    });
    const toolBlock = resp.content.find(b => b.type === "tool_use");
    if (!toolBlock) throw new Error("No tool_use block in response");
    const es = toolBlock.input;
    es.name = work.name;
    return es;
  }, 4, 2000);
}
...
async function main() {
  const {data:works} = await retry(async () => {
    const r = await sb.from("montree_classroom_curriculum_works")
      ...
      .eq("classroom_id",CID).is("guide_content_es",null).not("quick_guide","is",null)
      .order("name",{ascending:true});
    if (r.error) throw new Error(r.error.message);
    return r;
  }, 5, 3000);
  console.log(`Translating ${works.length} works to Argentine Spanish...`);
  let ok=0, fail=0, fails=[];
  for (let i=0; i<works.length; i++) {
    try {
      const es = await translateWork(works[i]);
      await retry(async () => {
        const {error} = await sb.from("montree_classroom_curriculum_works")
          .update({guide_content_es:es}).eq("classroom_id",CID).ilike("name",works[i].name);
