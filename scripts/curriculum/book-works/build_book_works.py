...
# 🚨 ONE SIDE-CAR TREE PER TRACK, never one that flip-flops (2026-09-15).
# The second-language track rewords every sentence, so a single directory
# would mean the committed transcript depended on how the print run was last
# invoked -- which is why side-cars used to be refused on that track outright.
# They are not refused any more, because the DIGITAL SHELF now ships the
# second-language wording (lib/montree/dark-phonics/book-works.ts BOOK_TRACK)
# and its conformance test needs a transcript of the pack that shelf matches.
# The two trees are separate files, so neither run can overwrite the other's:
#     scripts/curriculum/book-works/sidecars/<slug>.json                  first
#     scripts/curriculum/book-works/sidecars/second-language/<slug>.json  second
# Both are committed, both are regenerated by --sidecars-only (twice, once per
# --track), and each payload names its own track so a reader cannot mix them up.
SIDECAR_ROOT = os.path.join(HERE, 'sidecars')
SIDECAR_DIR = (os.path.join(SIDECAR_ROOT, 'second-language')
               if fw.is_second(TRACK) else SIDECAR_ROOT)
...
def sidecar_payload(slug, title, rows, flags, source, pages):
    ...
    return {
        ...
        # which wording this transcript is of -- see SIDECAR_DIR's note
        'track': 'second-language' if fw.is_second(TRACK) else 'first-language',
    }


def write_sidecar(slug, title, rows, flags, source, pages):
    """Written on BOTH tracks, into that track's own directory (SIDECAR_DIR),
    so the two transcripts sit side by side instead of overwriting each other.
    Before 2026-09-15 this refused the second-language track entirely; that was
    right while only the first-language wording was conformed to, and wrong the
    moment the digital shelf moved to the second-language wording."""
...
def emit_sidecars(slugs):
    ...
    Runs once per track and writes into that track's own directory; both trees
    are committed. Regenerate both after any source change:
        python3 build_book_works.py --sidecars-only
        python3 build_book_works.py --sidecars-only --track second-language
    """
    ok, skipped = 0, []
    for slug in slugs:
        try:
