    ...
    >
      <div style={{ maxWidth: 760, margin: '0 auto' }}>
        ...
        {migrationPending && (
          ...
          >
            ⚠️ {t('meetingNotes.migrationPendingTeacherIntro' as TranslationKey)}{' '}
            <code>migrations/214_meeting_notes.sql</code>{' '}
            {t('meetingNotes.migrationPendingTeacherOutro' as TranslationKey)}
          </div>
        )}
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          ...
          >
            {t('meetingNotes.teacherTitle' as TranslationKey)}
          </div>
          <div style={{ fontSize: 13, color: T.textSecondary, marginTop: 2 }}>
            {t('meetingNotes.teacherSubtitle' as TranslationKey)}
          </div>
        </div>
      </div>
    </div>
    ...
    try {
      ...
      if (data?.share) {
        ...
        if (data.share.threadId && !reason) {
          ...
          setShareInfo(t('meetingNotes.shareMessageFailedTeacher' as TranslationKey));
        }
      }
    } catch {
      setError(t('meetingNotes.errorNetwork' as TranslationKey));
