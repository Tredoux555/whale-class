      ...
      <div className="relative px-4 py-4 space-y-4">
        ...
        <div>
          ...
          <select
            ...
            className="w-full px-3 py-2 border border-[rgba(52,211,153,0.15)] rounded-lg bg-white/[0.06] text-white/90"
          >
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-sm font-medium text-white/70 mb-1">{t('albums.dateFrom')}</label>
            <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
              className="w-full px-3 py-2 border border-[rgba(52,211,153,0.15)] rounded-lg bg-white/[0.06] text-white/90" />
          </div>
          <div>
            <label className="block text-sm font-medium text-white/70 mb-1">{t('albums.dateTo')}</label>
        ...
        {totalPhotos > 0 && (
          <div className="space-y-4">
            ...
            {groups.map((group, gi) => (
              <div key={`${group.date}-${group.event_id || 'cur'}-${gi}`} className="bg-white/[0.06] border border-[rgba(52,211,153,0.15)] rounded-xl p-4">
                {/* Group header */}
                <div className="flex items-center gap-2 mb-3">
                  {group.event_name ? (
                    <>
                      ...
                      <div>
                        <span className="font-bold text-white/90">{group.event_name}</span>
                        <span className="text-white/50 text-sm block">{group.date}</span>
                      </div>
                    </>
                  ) : (
                    <span className="font-medium text-white/70">{group.date}</span>
                  )}
                ...
                <div className="grid grid-cols-3 gap-2">
                  {group.photos.map((photo) => {
                    ...
                    const url = urls[path];
                    return (
                      <div key={photo.id as string} className="aspect-square rounded-lg overflow-hidden bg-white/5">
                        {url ? (
