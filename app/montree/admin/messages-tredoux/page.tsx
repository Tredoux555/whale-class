// /montree/admin/messages-tredoux/page.tsx
// Migration 292 — Founding Principal ↔ Tredoux thread list + compose.
//
// Single recipient (Tredoux). Visual language mirrors the agent
// /messages-tredoux page for consistency; back-links target /montree/admin.
...
export default function PrincipalMessagesTredouxPage() {
  ...
  const load = async () => {
    try {
      const res = await fetch('/api/montree/principal/messages-tredoux/threads', { credentials: 'include' });
      if (res.status === 401) {
        router.replace('/montree/login-select');
        return;
      }
      if (res.status === 403) {
        // Not a founding school — this channel isn't available for them.
      ...
      console.error('[principal messages-tredoux] load failed', err);
      setLoading(false);
    }
  };
  ...
  const submitCompose = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/montree/principal/messages-tredoux/threads', {
        method: 'POST',
      ...
      await load();
      if (data.thread_id) {
        router.push(`/montree/admin/messages-tredoux/${data.thread_id}`);
      }
    } catch (err) {
      console.error('[principal messages-tredoux] send failed', err);
      ...
      <div style={{ position: 'relative', maxWidth: 720, margin: '0 auto', padding: '32px 20px 80px' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <p style={{ color: T.textSecondary, marginTop: 6, fontSize: 14 }}>
            Your direct line to Tredoux as a Founding school. Anything you need — questions, ideas, or something that isn&apos;t working.
          </p>
        </div>
          ...
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {threads.map((t) => (
                <Link
                  key={t.id}
                  href={`/montree/admin/messages-tredoux/${t.id}`}
                  style={{
                    display: 'block',
                    padding: 16,
          ...
          >
            ...
            >
              ...
              <input
                ...
                placeholder="e.g. A question about parent reports"
                maxLength={200}
                disabled={sending}
                style={{
