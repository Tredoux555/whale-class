// /montree/admin/messages-tredoux/[threadId]/page.tsx
// Migration 292 — Founding Principal ↔ Tredoux thread detail.
...
export default function PrincipalThreadDetailPage() {
  ...
  // Get the principal's own id from auth/me so we can detect "my" messages.
  useEffect(() => {
    fetch('/api/montree/auth/me', { credentials: 'include' })
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => { if (d?.teacher?.id) setMyPrincipalId(d.teacher.id); })
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [threadRes, messagesRes] = await Promise.all([
        fetch(`/api/montree/principal/messages-tredoux/threads/${threadId}`, { credentials: 'include' }),
        fetch(`/api/montree/principal/messages-tredoux/threads/${threadId}/messages`, { credentials: 'include' }),
      ]);
      ...
      if (threadRes.status === 403 || threadRes.status === 404) {
        router.replace('/montree/admin/messages-tredoux');
        return;
      }
      if (threadRes.ok) {
      ...
      setLoading(false);
      // Mark read on open
      void fetch(`/api/montree/principal/messages-tredoux/threads/${threadId}`, {
        method: 'PATCH',
      ...
      console.error('[principal thread detail] load failed', err);
      setLoading(false);
    }
  }, [threadId, router]);
  ...
  const send = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/montree/principal/messages-tredoux/threads/${threadId}/messages`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      console.error('[principal thread send] failed', err);
  ...
  const isMine = (m: Message): boolean => m.sender_role === 'principal';
