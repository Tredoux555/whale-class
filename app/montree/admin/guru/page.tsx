...
export default function GuruPage() {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`<h1>${modeTitle}</h1>`);
    printWindow.document.write(`<p class="subtitle">${selected?.classroom_icon} ${selected?.classroom_name} · ${schoolName}</p>`);
    ...
    if (guruResponse?.insight) {
      ...
      printWindow.document.write(`<p>${guruResponse.insight}</p>`);
    }
    ...
    if (reportMode === 'principal' && guruResponse?.root_cause && guruResponse.root_cause !== 'See insight above') {
      ...
      printWindow.document.write(`<p>${guruResponse.root_cause}</p>`);
    }
    ...
    if (guruResponse?.action_plan?.length) {
      ...
      guruResponse.action_plan.forEach((a, i) => {
        printWindow.document.write(`<div class="action-item"><span class="action-num">${i + 1}. ${a.action}</span>`);
        if (a.details) printWindow.document.write(`<p class="action-detail">${a.details}</p>`);
    ...
    if (guruResponse?.timeline) {
      ...
      printWindow.document.write(`<p>${guruResponse.timeline}</p>`);
    }

    // Parent talking point
    if (guruResponse?.parent_talking_point) {
      printWindow.document.write('<h2>Note for Parents</h2>');
      printWindow.document.write(`<div class="quote">"${guruResponse.parent_talking_point}"</div>`);
    }

    printWindow.document.write(`<div class="footer">Generated by Montree Guru · ${schoolName}</div>`);
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-900 via-emerald-800 to-teal-900 p-4 md:p-6">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {guruResponse?.success && !guruLoading && (
          <div ref={printRef} className="bg-white/10 rounded-2xl p-5 border border-emerald-500/30 mb-6 space-y-4">
        ...
        {guruResponse && !guruResponse.success && !guruLoading && (
          <div className="bg-red-500/10 rounded-2xl p-5 border border-red-500/30 mb-6 text-center">
            ...
            <button
              onClick={() => reportMode && generateReport(reportMode)}
