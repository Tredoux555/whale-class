...
// "I spy something that ends with /t/!"
...
export default function ISpyEndingGame() {
  ...
  const generateRound = useCallback((): GameRound => {
    // Pick a random ending sound group
    const soundGroup =
      ENDING_SOUNDS[Math.floor(Math.random() * ENDING_SOUNDS.length)];

    // Pick target word
    const targetWord =
      soundGroup.words[Math.floor(Math.random() * soundGroup.words.length)];

    // Get distractors from OTHER ending sounds
    const otherSounds = ENDING_SOUNDS.filter(
      (s) => s.sound !== soundGroup.sound
    );
    ...
    return {
      targetSound: soundGroup.sound,
      targetWord,
      options: allOptions,
    };
  }, []);

  // Play I Spy prompt for ending sound
  const playISpyPrompt = async (sound: string) => {
    setIsPlaying(true);

    const phonemePath = PHONEME_AUDIO[sound];
    if (phonemePath) {
      await soundGameAudio.speakWord('I spy something that ends with');
      await new Promise((r) => setTimeout(r, 300));
      await soundGameAudio.playPhoneme(phonemePath);
    } else {
      await soundGameAudio.speakWord(
        `I spy something that ends with ${sound}`
      );
    }
  ...
  const handleOptionSelect = async (selected: SoundWord) => {
    ...
    if (isCorrect) {
      ...
      setTimeout(async () => {
        // Emphasize the ending sound: "ca-T! Cat ends with /t/!"
        const word = selected.word;
        const ending = currentRound?.targetSound;
        await soundGameAudio.speakWord(
          `Yes! ${word} ends with ${ending}`
        );
      }, 500);
    } else {
      setFeedback({
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-500 flex items-center justify-center">
        <div className="text-center p-8 max-w-md">
          ...
          <p className="text-xl text-white/90 mb-8">
            Listen for the <span className="font-bold">LAST</span> sound in each
            word!
          </p>

          <div className="bg-white/20 backdrop-blur-sm rounded-2xl p-4 mb-8">
            <p className="text-white">
              <strong>Example:</strong> "ca-/t/" - Cat ends with /t/!
            </p>
          </div>

          <button
            onClick={startGame}
            className="w-full p-4 bg-white text-teal-600 rounded-2xl font-bold text-xl hover:bg-white/90 transition-all"
            style={{ fontFamily: 'Comic Sans MS, cursive' }}
          >
            Start Playing! 🎮
          </button>
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-500 flex items-center justify-center">
        <div className="text-center p-8">
          ...
          <div className="space-y-4">
            <button
              onClick={startGame}
              className="w-full max-w-sm mx-auto p-4 bg-white text-teal-600 rounded-2xl font-bold text-xl"
              style={{ fontFamily: 'Comic Sans MS, cursive' }}
            >
              Play Again! 🔄
            </button>
