  ...
  try {
    ...
    if (childId) {
      ...
      const [
        ...
        supabase.from('montree_media').select('id, storage_path, thumbnail_path, media_type, caption, captured_at, child_id, work_id, parent_visible, school_id, classroom_id, created_at, updated_at, auto_crop, tags').eq('child_id', childId).or('identification_status.is.null,identification_status.neq.pending_review').order('captured_at', { ascending: false }).limit(500),
