...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // AUDIT-FIX: previously returned a distinct 404 {error:'not_found'} here,
    // which — unlike every other DPL route's 404-hides-existence pattern —
    // sat AFTER this route's own code/expiry/max-uses checks (which already
    // return 401 on a bad code). That made the flag gate a same-endpoint
    // oracle: a valid-code-but-flag-off code would 404 while an invalid code
    // 401'd, letting an attacker confirm a code is real at a school that
    // simply hasn't enabled the feature yet. Now it fails exactly like an
    // invalid code, so this endpoint reveals nothing about the flag OR the
    // code's validity beyond what the invalid-code branch already reveals.
