...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (const classroom of classrooms) {
      ...
      if (classroomError) {
        console.error(`[Setup] Classroom creation error for "${classroom.name}":`, classroomError);
        continue;
      }
      ...
      for (const teacher of teachersToCreate) {
        ...
        if (teacherError) {
          console.error(`[Setup] Teacher creation error for "${teacher.name}":`, teacherError);
          continue;
        }
