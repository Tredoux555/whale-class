...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if a draft report with AI narrative already exists for this week
    // (generated by batch-narratives endpoint)
    const existingNarrative = await (async (): Promise<{ summary: string; generated_at: string; model: string } | null> => {
      const weekStartDate = new Date(weekStart);
      const yr = weekStartDate.getFullYear();
      const soy = new Date(yr, 0, 1);
      const dsd = Math.floor((weekStartDate.getTime() - soy.getTime()) / (24 * 60 * 60 * 1000));
      const wn = Math.ceil((dsd + soy.getDay() + 1) / 7);

      const { data: draftWithNarrative } = await supabase
        .from('montree_weekly_reports')
        .select('content')
        .eq('child_id', child_id)
        .eq('week_number', wn)
      ...
      const draftContent = (draftWithNarrative as Record<string, unknown> | null)?.content as Record<string, unknown> | null;
      if (draftContent?.narrative) {
        return draftContent.narrative as { summary: string; generated_at: string; model: string };
      }
      return null;
    })();
    ...
    const reportContent = {
      ...
      closing,
      works: allWorks,
      // AI narrative (if previously generated via batch-narratives)
      ...(existingNarrative ? { narrative: existingNarrative } : {}),
    ...
    if (parentEmails.length > 0 && process.env.RESEND_API_KEY) {
      ...
      // Get the saved report ID for the "View Full Report" link
      ...
      const savedReport = savedReportRaw as { id: string } | null;
      const reportLink = savedReport?.id
        ? `https://montree.xyz/montree/parent/report/${savedReport.id}`
        : '';

      // If we have an AI narrative, use the teaser + link email format
      // Otherwise fall back to the classic work-list email
      const narrativeSummary = existingNarrative?.summary || '';
      const html = narrativeSummary && reportLink
        ...
        <div style="font-family: -apple-system, sans-serif; max-width: 500px; margin: 0 auto; padding: 20px;">
          ...
          <p style="color: #999; font-size: 12px; text-align: center;">Montree · ${classroom?.name || ''}</p>
        </div>
        `
        : `
        <div style="font-family: -apple-system, sans-serif; max-width: 500px; margin: 0 auto; padding: 20px;">
    ...
    try {
      ...
      void pushToParentsOfChildren(
        ...
        [child_id],
        {
          ...
          body: `${child.name}'s progress report is ready to view.`,
