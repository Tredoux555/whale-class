...
export async function POST(request: NextRequest) {
  try {
    ...
    // 3b: Translate into all enabled locales — fire-and-forget via Haiku tool_use.
    // Only for NEW custom works (not deduplicated — those may already have translations).
    if (!deduplicated) {
      translateAllLocales({
        classroomId,
        ...
        console.error('[AddCustomWork] Background translation failed (non-fatal):', err instanceof Error ? err.message : err);
      });
    }
