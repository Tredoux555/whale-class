...
function decryptFile(encryptedBuffer: Buffer, password: string): Buffer {
  try {
    ...
    console.error('[Vault Decrypt] Decryption error:', error);
...
export async function GET(
  ...
  try {
    ...
    if (downloadError || !encryptedData) {
      console.error('[Vault Download] Download error:', downloadError);
    ...
    await dbQuery(
      `INSERT INTO vault_audit_log (action, admin_username, ip_address, details, success)
       VALUES ($1, $2, $3, $4, TRUE)`,
      ['file_download', adminUsername, ipAddress, `File: ${file.filename}`]
    );
    ...
    await dbQuery(
      ...
      ['file_download', 'unknown', ipAddress, error instanceof Error ? error.message : 'Unknown error']
    ).catch(() => {});
