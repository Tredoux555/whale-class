'use client';

import { useEffect } from 'react';

// Redirect-only page. The CSP header on montree.xyz blocks all iframes
// (frame-ancestors 'none'), so the admin page can't iframe the static
// HTML. Instead we redirect straight to the static file.
export default function ReadingContentGreenPage() {
  useEffect(() => {
    window.location.replace('/whale-reading-content-green.html');
  }, []);

  return (
    <div style={{
      minHeight: '100vh',
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center',
      background: '#0a1a0f',
      color: '#e8e8d8',
      fontFamily: 'Inter, -apple-system, sans-serif',
    }}>
      <div style={{ textAlign: 'center' }}>
        <div style={{ fontSize: 48, marginBottom: 16 }}>📗</div>
        <div style={{ fontSize: 18, marginBottom: 8 }}>Opening Green Phase lesson content…</div>
        <div style={{ fontSize: 13, color: '#888', marginBottom: 16 }}>If this doesn&rsquo;t redirect automatically:</div>
        <a
          href="/whale-reading-content-green.html"
          style={{
            color: '#1f6f3e',
            textDecoration: 'underline',
            fontSize: 14,
          }}
        >
          Open the lesson content directly →
        </a>
      </div>
    </div>
  );
}
