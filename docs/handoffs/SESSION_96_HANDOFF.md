# Session 96 — Astra Cockpit Float, Principal Drill-Down Redesign, Privacy Fix, Free-Tier Degradation, Welcome Template

**Date:** May 8, 2026 (evening)

**Scope:** Big push on the principal-as-overseer experience. Astra went from a single-page chat to a cockpit-wide chief-of-staff float. The classroom drill-down was redesigned from heavy-gradient/dual-priority-sections to a calm overseer-mental-model layout. Conversation history was leaking between schools — fixed via per-school storage namespacing. Astra now degrades gracefully on Free-tier schools instead of showing a red error. The teacher welcome template was meaningfully upgraded with PWA install instructions and a Guru pointer. One stacking-context bug fixed at the very end (kebab dropdown was being hidden behind the next teacher row).
...
| `575b29cb` | Astra: cockpit-wide float + draft tool + terse chief-of-staff voice |
| `d0188438` | Astra: Opus voice + first-meeting introduction protocol |
| `926d5531` | Classroom drill-down: redesign for principal-as-overseer flow |
| `451dc548` | Astra: school-scoped storage + Free-tier graceful degradation + better welcome |
...
### Astra as cockpit-wide chief-of-staff (`components/montree/admin/TracyFloat.tsx`)

- New floating component injected into `app/montree/admin/layout.tsx`. Visible on every cockpit page except `/montree/admin` (chat page IS Astra in full there).
...
- Question-form action lines ending in `?` surface inline `Yes, please` / `Not now` buttons that auto-send back to Astra (Pattern A — clean conversation flow, no special UI state).
- Conversation state persists across page navigation via localStorage.

### Astra's voice — Opus + prose-style system prompt + first-meeting protocol
...
- System prompt rewritten as natural prose describing who Astra is, with rules embedded as natural consequences of her character rather than commandments shouted in caps. Opus rewards prose framing with significantly warmer voice.
- Anti-AI-tells list baked into the prompt — Astra avoids "I had a look around", "I noticed that", "Based on what I'm seeing", "Hope this helps", etc.
...
- Astra's first ACTION tool (vs the existing read-only ones). Generates copy-paste-ready welcome messages with each teacher's login code, school name, classroom name, and principal sign-off.
...
### School-scoped Astra storage (`lib/montree/tracy/storage-keys.ts`)

The privacy fix. Every Astra localStorage / sessionStorage key is now scoped by `school_id`. Logging into different schools on the same browser will never bleed conversations again.
...
> Hi, I'm Astra. I'll be your assistant once your school's AI features are switched on — Montessori expert in your pocket, drafting messages for your teachers, helping you handle parent questions, all the school operations work.
>
...
Classroom-page Send button (`sendEmailToTeacher`) and Astra's `draft_teacher_welcome_messages` tool now produce the same warmer template:
...
PWA install instructions are explicit (iOS + Android both covered). Guru is named directly so the teacher knows who to ask once they land. Whether the principal sends from the classroom row or asks Astra to draft for the whole team, the teacher gets identical text — feels like one product.
...
1. **Astra is the principal's only AI chat surface.** Guru is per-child Maria-Montessori-in-pocket for teachers. Astra can call Guru as a sub-tool when child-pedagogical depth is needed (`consult_guru` not yet implemented but reserved).
2. **Astra runs on Opus.** All other principal-side AI stays on Sonnet. The OPUS_MODEL constant in `lib/ai/anthropic.ts` is what the principal-agent route imports. To revert: swap `OPUS_MODEL` for `AI_MODEL` and update cost constants.
...
4. **Astra's voice rules** — short, smart, no narration of process, principal-as-overseer reframe, end with one concrete next move. Two distinct kickoff prompts (`[GREETING_FIRST]` introduction + `[GREETING]` short greeting). The `→ ` arrow marker is load-bearing — front-end parses it.
...
7. **Welcome message template lives in TWO places** (classroom-page Send button + Astra's draft tool) and they MUST stay in lockstep. Both files have a comment pointing at the other. Edit both in the same commit.
...
- No team-level "what did we do this week" surface yet. Astra's `unpack_teacher` is per-teacher; a team-level read isn't a current tool.
...
4. Verify the conversation leak is gone — Astra on Test School 1 should show fresh thread, not Whale Class's Amy chat.
5. Tap a teacher's Send button — mailto should open with the new welcome template (PWA install tip + Guru pointer + sign-off).
6. To verify Free-tier degradation: temporarily flip Test School 1 back to Free in super-admin, hard refresh, watch Astra auto-open with static welcome instead of red error. Then flip back to Premium.
...
1. **Astra float overlap on viewports < ~1330px.** Page content extends into the Astra panel zone. Layout-shift work — reserve right padding when float is open, or shrink page maxWidth when float is open. ~30 min.
2. **Continue the dashboard redesign page-by-page** following the overseer mental model:
   ...
   - Today (`/montree/admin`) — architectural decision: stays as Astra chat, or becomes a digest with Astra floating? My lean: digest. ~1.5h plus design discussion.
...
10. **Optional: GuruFloat** — the teacher-side mirror of TracyFloat, using everything we built for Astra as a template (~2-3h). Build when teacher onboarding signal indicates they're getting lost.
...
- Astra on Opus: ~$0.20 per interaction (was $0.04 on Sonnet).
- At 3-5 Astra interactions/day per active principal: ~$1/day per principal.
...
- **Astra can answer parent-relayed questions** (`child_focus` tool with the parent's question text). She drafts the response in the principal's voice. But it's the principal who relays — there's no in-app channel.
...
6. **How does notification work?** Email + push notification + in-app badge? Cross-device sync? Parents probably want push (they're on mobile, the PWA install we just put in the welcome email becomes critical). Teachers probably want email + in-app (they're at work). Principal wants in-app surfacing via Astra.
...
- Astra gets a new tool `summarise_parent_threads` for the principal's overview.
...
- Don't build code. Theorize the data model, the channels, the routing rules, the role of Astra and Guru. Decision document, not a feature.
