...
- ✅ Migration 184 — `montree_principal_agent_log` table created. Tracy interactions now log to it (no more silent failure).
...
- Your actual pain points are Tracy stutter on long answers, VPN drops losing conversation state, and returning to a cached page that shows a spinner
...
### Day 2 — Bundle + Tracy smoothness (~5-6 hours)

**Goal:** Cut bundle size for everyone + make Tracy feel native during streaming.
...
3. **Tracy SSE token throttle via `requestAnimationFrame`** (~1.5 hours)
   ...
   - Win: ~80% CPU reduction during streaming. Smooth scroll while Tracy types long answers.
   - **Alternative noted:** React 19's `useTransition` around buffer flush — explore as 30-min spike before committing to rAF plumbing.

**One commit at end of Day 2.** Title: "Lora next/font + cookie locale dispatch + Tracy rAF throttle"

**At end of Day 2, check web-vitals dashboard.** If FCP didn't drop ≥200ms or INP during Tracy streaming didn't drop ≥50%, something's wrong and worth investigating before Day 3.
...
1. **Tracy SSE AbortController + retry-with-resume** (~4-6 hours)
   ...
   - Test by toggling Astrill mid-Tracy-response: conversation should auto-recover, not error out.
...
**One commit at end of Day 3.** Title: "Tracy SSE resilience + montreeApi auto-retry + Weekly Wrap parallel teacher/parent reports"
...
- INP during Tracy streaming (rAF throttle)
- Stream resilience (Tracy survives VPN drops in manual testing)
...
- Tracy 2.3 (skip Sonnet greeting on first paint) — 2h, removes the "thinking dots on Tracy expand" lag
...
| **Wholesale `fetch()` audit (~80 sites)** | Defer. Phase A (top 5 endpoints) covered in Day 3 via Tracy + `montreeApi`. Rest is organic refactor as files are touched. |
...
3. **Tracy production testing** — verify memory works end-to-end, thinking indicator pulses, copy cards render. Test plan in Session 100 handoff Section "Test plan for the next session".
...
The performance plan is documented in `docs/PERF_HEALTH_CHECK.md`. It's comprehensive (55-65h, audited twice) but more than current scale needs. Trimmed rollout is **~15-18 hours across 3 comfortable days**, targeting the actual felt pain points (Tracy stutter, VPN resilience, bundle size, Lora paint waterfall). After Day 3, measure with Web Vitals and decide whether to continue. The other ~40 hours of plan is ready to ship when scale or real complaints justify it.
