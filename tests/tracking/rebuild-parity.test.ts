...
// transcription of 348's CTE, and it must agree with the engine on every fixture.
// If you change one, this test tells you to change the other.
...
export function sqlRebuildMirror(
  ...
  // ROW_NUMBER() OVER (PARTITION BY COALESCE(work_key, work_name)
  //                    ORDER BY created_at DESC) = 1
  const latest = new Map<string, ProgressEvent>();
  for (const e of accepted) {
    const partition = e.work_key ?? e.work_name;
    const held = latest.get(partition);
    if (!held || Date.parse(e.created_at) >= Date.parse(held.created_at)) latest.set(partition, e);
  }

  // DISTINCT ON (work_name) … ORDER BY work_name, created_at DESC
  const byName = new Map<string, ProgressEvent>();
  for (const e of latest.values()) {
    const held = byName.get(e.work_name);
    if (!held || Date.parse(e.created_at) > Date.parse(held.created_at)) byName.set(e.work_name, e);
  }
...
describe('rule 3 — the SQL rebuild and the engine rebuild derive the same statuses', () => {
  ...
  it('the §3b abort condition is gone: two keys under one work_name yield ONE row', () => {
    // migration 346 partitioned by work_key and then INSERTed ON CONFLICT
    // (child_id, work_name), so this pair produced two rows aimed at one conflict
    // target — Postgres 21000, and the whole child aborted. 348 collapses to one
    // row per work_name, so the mirror must too.
    const events = [
    ...
    expect(mirrored.size, 'one row per work_name reaches the INSERT').toBe(1);
  });
