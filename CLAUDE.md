...
**Fix 1 — verify link lands in the app (`b703518d`, 1 file).** `GET /api/lyf-coach/verify` verifies + sets the httpOnly `story-admin-token` cookie + redirects to `/lyf-coach/coach`, but the `(app)` layout guard authed ONLY from sessionStorage (which only signup/login populate) → freshly-verified users bounced to `/lyf-coach/login`. The promised "cookie→sessionStorage bridge" was never wired (the backend `GET /api/lyf-coach/session` existed; nothing called it). Fix in `app/lyf-coach/(app)/layout.tsx`: when sessionStorage has no token, call `/api/lyf-coach/session` (reads the cookie → returns token), mirror into sessionStorage, proceed; bounce to login only if that also fails. Existing logged-in users hit sessionStorage first (no extra fetch). 🚨 Bare `/lyf-coach` is the public SIGNUP page, not the app — redirect target is intentionally `/lyf-coach/coach`.

**Fix 2 — first-login welcome banner (`8c36b299`, server-backed).** Migration **272** adds `first_login_shown boolean default false` to `story_admin_users`. NEW `app/api/lyf-coach/welcome/route.ts`: GET decides `{show,variant}` read-only (`show=email_verified && !first_login_shown`; `variant=welcome_bonus_period?'founder':'standard'`), POST atomically stamps `first_login_shown=true WHERE first_login_shown=false` (idempotent/race-safe), auth = Bearer or cookie. NEW `components/story/lyf-coach/WelcomeBanner.tsx`: full-width, dismiss-on-click, POST-stamps the moment it renders; wired into the `(app)` layout above VerifyEmailBanner (final styling per follow-up (b) below). **Removed** the old localStorage founder banner from the coach page (no double banner). Founder copy = "…one of the first 100. You have 1000 prompts this month. Welcome." / standard = "Welcome to Lyf Coach. Your 7-day free trial starts now. Let's begin." Migration 272 is RUN (Jun 24) → banner LIVE. 🚨 Existing verified users see it once on next load (column default false) — one-line backfill (`UPDATE story_admin_users SET first_login_shown=true WHERE email_verified=true;`) to suppress if undesired.

**🔧 Follow-ups (same session, 2 more commits):** (a) **One login path (`2153d91c`)** — public coach surfaces bounced to `/lyf-coach/login`, but the shared `personal-client.bounceToLogin` + `use-voice-record` still hardcoded `/story/admin` (the owner Sanctuary login), so a logged-out public user got BOTH the layout's `/lyf-coach/login` AND a shared-module `/story/admin` redirect ("two login paths"). Extracted one path-aware helper `lib/story/login-path.ts` (`coachLoginPath()` → `/lyf-coach/login` when path starts `/lyf-coach`, else `/story/admin`) and routed `personal-client`, `use-voice-record`, AND the coach chat provider (which already had an inline copy) through it. Public → one path; owner Sanctuary unchanged. (b) **Welcome banner restyle (`8324a909`)** — the Fix 2 banner shipped as a loud solid-`#00a86b`/white block; restyled to match the original founder banner's classy look: soft emerald tint `rgba(52,211,153,0.09)`, thin emerald border, `T.emerald` text, 14px, soft × at 0.65. 🚨 This intentionally OVERRIDES the original Fix 2 spec (`#00a86b`/white) per Tredoux's "cleaner and more classy" — the canonical banner style is now the emerald-tinted card, NOT solid green. Two-variant copy + dismiss + server `first_login_shown` logic unchanged.

**🅿️ Parked (NOT shipped):** (1) **Multi-photo (≤5) + document upload** for the coach composer — in `coach_uploads.patch` (repo root + the Desktop backup); adds dep **`unpdf`**; `git apply` onto a fresh origin/main worktree + `npm install` to ship. (2) **App-Store account-deletion endpoint** — on branch `keep/lyfcoach-account-deletion` (`bf346dff`, `app/api/story/admin/account/route.ts`, not on main); its table list predates the family/billing tables — reconcile vs `app/api/montree/super-admin/lyf-coach/member/route.ts` before shipping.

**🧹 Home-checkout cleanup.** The `montree` checkout had drifted to 47-behind/5-ahead — parked on `account-deletion-jun19` since Jun 20 while sessions shipped via throwaway worktrees. Saved `bf346dff`→`keep/lyfcoach-account-deletion`, backed up private/local files to `~/Desktop/montree-cleanup-backup-2026-06-24/` (`social/` + 3 docs + patch), `stash -u` (`stash@{0}`) → `checkout main` + `reset --hard origin/main` → restored private files, deleted 8 merged dead branches, cleared a stale `.git/index.lock`. End state: clean `main` == `origin/main`, private files present, safety nets (keep/ + stash@{0} + Desktop backup) intact. **🚨 Standing rule to stop recurrence: after each push, re-sync the home checkout (`git checkout main && git fetch && git reset --hard origin/main`) — keep isolated worktrees for pushes, never park the home checkout on a feature branch.**
