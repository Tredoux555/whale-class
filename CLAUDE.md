...
**Sheet 14 — Sentence Builder, 18 illustrated cards, 10 pages (5 duplex sheets).** Three tiers, and the FRAME COLOUR IS THE TIER (Montessori pink/blue/green reading series; no written difficulty label anywhere on the card): 6 pink = pure three-letter CVC (`the cat sat`, `the ant sat`, `the sun sat`, `the ant is sad`, `the ant is hot`, `a pig in a wig`), 6 blue = four-letter words (`a fox in a box`, `the ant naps`, `the ant digs`, `the cat naps`, `the cat digs`, `the sun naps`), 6 green = one consonant blend a card (`the star sat` st, `the penguin spat` sp, `the blob sat` bl, `the crab sat` cr, `the sad dad sat in the sand` nd, `the cat can jump` mp). Green is complete at six. Picture on the FRONT alone, sentence on the BACK alone (Montessori three-part card — a picture with its words beside it is a label). Every sentence is lower case with no full stop: the capital and the stop are the child's to add with a punctuation tile. Art is 15/18 REUSED (the-sat tiles, the-sad, the-hot, the-nap, the-dig, the-spat, story starters, w08-sand); only blob, crab and jump were drawn for the set. Art is TRIMMED TO ITS INK at build time and fitted to the content box, centred on the ink; source files are never written to.

**Sheet 13 — Story Starter cards, 14 cards, 8 pages (4 duplex sheets).** Same card, same frame. Phrases are literal short phrases (`hen in a pen`, NOT "The hen is in a pen"), all lower-case initial. Framed deck-wide in warm charcoal `#5A5248` (a neutral, NOT a fourth tier) except `hen-pen`, which carries sheet 14's tier-1 pink as a level marker the teacher finds by eye.

**CARD DESIGN CONSTANTS — do not reinvent.** Defined once in `build_14_sentence_builder_cards.py` and imported constant-for-constant by `build_13`: card 80 × 120 printed → 100 × 140 mounted; rounded frame, 5 mm outer corner radius, 2.6 mm stroke, outer edge 4 mm off the cut line on all four sides; 0.4 mm hairline of the same colour 2.2 mm inside it; content box 60 × 100 mm, even all round. Type is COMIC NEUE (free SIL-OFL; Comic Sans MS is not embeddable) at ONE deck em of **10.90 mm shared across BOTH decks** — a longer sentence takes more lines, never smaller type; adult margin text stays Andika. Duplex SHORT EDGE, back grid mirrored top-to-bottom with each back drawn rotated 180°. Tier colours: pink `#D45B86`, blue `#2F5FA6`, green `#2F7D4F`; sheet 13 neutral `#5A5248`. Sheet 14's backing card is pink/blue/green by tier — on that sheet the colour IS the reading level.
...
**Reprint list:** sheets 12 (3 pp), 13 (8 pp), 14 (10 pp) = 21 pages. **Open:** five wide (3:2) sources — the four the-sat tiles and w08-sand — still letterbox on the tall card; `blob` prints at ~169 dpi (subject small in its source); Story Books sets D (butterfly) and E (frog) are photos and it is STILL UNDECIDED whether to colour-correct them to the pure white of A/B/C; the manifest's summary `stock` block and the sheet-14 paragraph in `scripts/curriculum/writing-shelf/README.md` are stale. **Floated, not started:** a "blend box" reference the child consults for an unfamiliar blend, and a later freestyle word-building stage.
