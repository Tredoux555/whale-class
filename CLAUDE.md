...
## 🌍 SESSION — Jul 6, 2026 (Cowork/Fable, evening) — GLOBAL OUTREACH SCRAPE session 1/3 + CLAUDE.MD ARCHIVED (1MB→147KB)

**Canonical: `docs/outreach/OUTREACH_MASTER_TRACKER.md` (status + session-2 queue + source intel) + `docs/handoffs/HANDOFF_GLOBAL_OUTREACH_SCRAPE.md`. No code, no migrations — data + docs only.**

- **11 country lists built, ~1,900 schools, ~1,730 verified emails (~91%), every domain MX-checked:** UK 71 · Germany 501 (assoc-affiliated COMPLETE) · Netherlands 214 (COMPLETE) · Australia+NZ 532 · Ireland 64 · UAE 49 · Canada 201 (CCMA COMPLETE) · Italy 303 (ONM registry COMPLETE) + disadvantaged-schools side-list seeded (37 rows, 20 countries — grows with Africa/Asia). CSVs in `docs/outreach/<country>/`.
...
- **🚨 RULE — outreach sub-agents run `model:"sonnet"`** (Fable orchestrates only). List rules: emails only when SEEN (never constructed); MX-check via dig (zero AI); Gmail `to:DOMAIN in:sent` dedup stays mandatory at draft time; NEVER auto-send.
- **NEXT (session 2 of 3, fresh session):** France/Spain/Japan/Korea/India/LatAm + top-ups via parallel Sonnet agents — queue + per-country source intel in the tracker. Session 3: USA (state-by-state) + Africa + final dedup → Campaign Manager drafting.
